...
class ModelTrainer:
    ...
    def train(
            ...
            # train_with_marginal: bool = False,
            **kwargs,
    ) -> dict:
        """
        ...
        # log.info(f' - train_with_marginal: "{train_with_marginal}"')
        log_line(log)
            ...
            for epoch in range(epoch + 1, max_epochs + 1):
                ...
                for batch_no, batch in enumerate(batch_loader):
                    ...
                    for batch_step in batch_steps:
                        ...
                        # forward pass
                        # if train_with_marginal:
                        #     loss = self.model.marginal_loss(batch_step)
                        # else:
    ...
    def distill(
        ...
        if aug_dataset:
            ...
            length_2_list_of_sents = defaultdict(list)
            for sentence in self.corpus.train:
                length_2_list_of_sents[len(sentence)].append(sentence)

            proxy_sents = []

            print('Process proxy data ...')
            try:
                for aug_sent in tqdm(aug_dataset):
                    length = len(aug_sent)
                    while length and length not in length_2_list_of_sents:
                        length -= 1
                    if length > 0:
                        train_sent = random.choice(length_2_list_of_sents[length])
                    else:
                        print('not found')
                        train_sent = random.choice(self.corpus.train)

                    long_sent, short_sent = (train_sent, aug_sent) if len(
                        train_sent) > len(aug_sent) else (aug_sent, train_sent)

                    sentences = [long_sent, short_sent]

                    self.model.embeddings.embed(sentences)

                    names = self.model.embeddings.get_names()

                    lengths: List[int] = [len(sentence.tokens) for sentence in sentences]
                    longest_token_sequence_in_batch: int = max(lengths)

                    pre_allocated_zero_tensor = torch.zeros(
                        self.model.embeddings.embedding_length * longest_token_sequence_in_batch,
                        dtype=torch.float,
                        device=flair.device,
                    )
                    ...
                    for sentence in sentences:
                        ...
                        if nb_padding_tokens > 0:
                            t = pre_allocated_zero_tensor[
                                : self.model.embeddings.embedding_length * nb_padding_tokens
                            ]
                            all_embs.append(t)
                    ...
                    sentence_tensor = torch.cat(all_embs).view(
                        [
                            ...
                            self.model.embeddings.embedding_length,
                        ]
                    )

                    marginal = [torch.cat((sentence._marginal, torch.zeros(
                        longest_token_sequence_in_batch - sentence._marginal.size()[0],
                        len(self.model.tag_dictionary.item2idx)))) for sentence in sentences]
                    marginal = torch.stack(marginal, 0)

                    alpha = 0.2

                    new_emb = sentence_tensor[0] * alpha + sentence_tensor[1] * (1 - alpha)
                    new_marginal = marginal[0] * alpha + marginal[1] * (1 - alpha)

                    proxy_sent = copy.copy(long_sent)
                    proxy_sent.proxy_emb = new_emb.flatten().detach()
                    proxy_sent._marginal = new_marginal.detach()
                    proxy_sents.append(proxy_sent)
                    # pdb.set_trace()

                aug_dataset.sentences = proxy_sents
        ...
        # pdb.set_trace()
        # if training also uses dev/train data, include in training set
        if train_with_dev or train_with_test or aug_dataset:
            ...
            for epoch in range(epoch + 1, max_epochs + 1):
                ...
                for batch_no, batch in enumerate(batch_loader):
                    ...
                    for batch_step in batch_steps:
                        ...
                        feature = self.model.forward(batch_step)
                        distill_loss = self.model.distill_loss(feature, batch_step)
                        loss = distill_loss



                        # feature = self.model.forward(batch_step)
                        # crf_loss = self.model._calculate_loss(feature, batch_step)
                        # distill_loss = self.model.distill_loss(feature, batch_step)
                        # loss = 0.5 * distill_loss + 0.5 * crf_loss
    ...
    def train_balanced_batch(
        ...
        assert aug_dataset is not None, 'Augment dataset cannot be none.'
        assert mini_batch_size % 2 == 0, 'mini_batch_size must can be divided by 2.'
        ...
        try:
            ...
            for epoch in range(epoch + 1, max_epochs + 1):
                ...
                aug_loader = DataLoader(
                    aug_dataset,
                    batch_size=mini_batch_size // 2 if epoch > 1 else mini_batch_size,
                    shuffle=True,
                    num_workers=num_workers,
                    sampler=sampler,
                )
                aug_batch_iter = iter(aug_loader)
                ...
                for batch_no, batch in enumerate(batch_loader):
                    ...
                    try:
                        aug_batch = next(aug_batch_iter)
                    except StopIteration:
                        aug_batch_iter = iter(aug_loader)
                        aug_batch = next(aug_batch_iter)

                    if epoch > 1:
                        batch = batch + aug_batch   
                    else:
                        batch = aug_batch
...
class ModelFinetuner:
    ...
    def train(
        ...
        if inspect.isclass(optimizer):
            ...
            bert_param_optimizer = list(self.model.embeddings.named_parameters())
            ...
            optimizer_grouped_parameters = [
                ...
                {'params': self.model.transitions,'weight_decay': 0.01, 'lr': learning_rate * 100},

                {'params': [p for n, p in linear_param_optimizer if not any(nd in n for nd in no_decay)],
                'weight_decay': 0.01, 'lr': learning_rate * 100},
                {'params': [p for n, p in linear_param_optimizer if any(nd in n for nd in no_decay)], 'weight_decay': 0.0,
                'lr': learning_rate * 100}
            ]


            optimizer: torch.optim.Optimizer = optimizer(optimizer_grouped_parameters, lr=learning_rate, **kwargs)
            ...
            for epoch in range(epoch + 1, max_epochs + 1):
                ...
                for batch_no, batch in enumerate(batch_loader):
                    ...
                    for batch_step in batch_steps:
                        ...
                        feature = self.model.forward(batch_step)
                        # crf_loss = self.model._calculate_loss(feature, batch_step)
                        # if isinstance(crf_loss, Tuple):
                        #     average_over += crf_loss[1]
                        #     crf_loss = crf_loss[0]

                        distill_loss = self.model.distill_loss(feature, batch_step)

                        # loss = distill_loss + crf_loss
                        loss = distill_loss
