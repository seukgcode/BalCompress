...
class SequenceTagger(flair.nn.Classifier):
    ...
    def assign_softtarget(self, sentences, distill_storage_mode='cpu'):
        ...
        if not self.use_crf:
            ...
            logits[:, :, self.tag_dictionary.get_idx_for_item('<unk>')] -= 1e12
            length = torch.Tensor([len(sentence.tokens) for sentence in sentences])
            max_len = max(length)
            length = length.long()
            mask = self.sequence_mask(length, max_len).unsqueeze(-1).to(flair.device).long()
            forward_var = self._forward_alg(logits, length, distill=True)
            backward_var = self._backward_alg(logits, length, distill=True)
            forward_backward_score = (forward_var + backward_var) * mask.float()
        
        # pdb.set_trace()
        for idx, sentence in enumerate(sentences):
            sentence.set_teacher_marginal(
                forward_backward_score[idx][:len(sentence)], 
                distill_storage_mode
            )
    ...
    def distill_loss(
        ...
        lengths = torch.Tensor([len(sentence.tokens) for sentence in sentences])
        tag_size = len(self.tag_dictionary.idx2item)
        max_len = int(max(lengths).item())
        lengths = lengths.long()
        mask = self.sequence_mask(lengths).unsqueeze(-1).to(flair.device).long()
        try:
            teacher_score = [torch.cat((sentence._marginal, torch.zeros(max_len - sentence._marginal.size()[0], tag_size))) for sentence in sentences]
            teacher_score = torch.stack(teacher_score, 0).to(flair.device) * mask
        except:
            pdb.set_trace()

        forward_var = self._forward_alg(features, lengths, distill=True)
        backward_var = self._backward_alg(features, lengths, distill=True)
        student_score = (forward_var + backward_var) * mask
        # pdb.set_trace()
        loss = F.kl_div(F.log_softmax(student_score, dim=-1), F.softmax(teacher_score, dim=-1), reduction='batchmean')
    ...
    def marginal_loss(
        ...
        # teacher_score = [torch.cat((sentence._marginal, torch.zeros(
        #     max_len - len(sentence), tag_size))) for sentence in sentences]
        # teacher_score = torch.stack(teacher_score, 0).to(flair.device) * mask

        forward_var = self._forward_alg(features, lengths, distill=True)
        backward_var = self._backward_alg(features, lengths, distill=True)
        score = (forward_var + backward_var) * mask
        ...
        for s_id, sentence in enumerate(sentences):
            ...
            ] + [self.tag_dictionary.get_idx_for_item('<unk>')] * (max_len - lengths[s_id])
            # add tags as tensor
            tag = torch.tensor(tag_idx, device=flair.device)
            tag_list.append(tag)
        labels = torch.stack(tag_list, dim=0)

        loss = F.cross_entropy(score.view(-1, tag_size), labels.view(-1),
                               ignore_index=self.tag_dictionary.get_idx_for_item('<unk>'))
    ...
    def forward(self, sentences: List[Sentence]):
        ...
        for sentence in sentences:
            if hasattr(sentence, 'proxy_emb'):
                all_embs += [sentence.proxy_emb]
            else:
                all_embs += [
    ...
    def _forward_alg(self, feats, lens_, distill=False):
    ...
    def _backward_alg(self, feats, lens_, distill=True):
        ...
        terminal_var = forward_var + bw_transitions[
            self.tag_dictionary.get_idx_for_item(START_TAG)
        ][None, :].repeat(forward_var.shape[0], 1) + reversed_feats[range(reversed_feats.shape[0]), lens_-1, :]
