...
class FastChineseWordEmbeddings(TokenEmbeddings):
    """Standard static word embeddings, such as GloVe or FastText."""

    def __init__(self, embeddings: str, datasets: list, max_len: int=3, field: str = None, fine_tune: bool = False, force_cpu: bool = True,
                 stable: bool = False):
        """
        ...
        if 'tencent' in str(embeddings):
            precomputed_word_embeddings = KeyedVectors.load_word2vec_format(str(embeddings), binary=False)
        ...
        for dataset in datasets:
            for sent in tqdm(dataset):
                ...
                tokens_set = set(
                    ''.join(tokens[token_idx: token_idx + word_len]) \
                    for word_len in range(1, max_len + 1) \
                    for token_idx in range(0, len(tokens), word_len)
                )
                ...
                for token in tokens_set:
                    ...
                    if token not in vocab_tmp:
                        embeddings_tmp.append(word_embedding.unsqueeze(0))
                        vocab_tmp[token] = len(vocab_tmp)
                # pdb.set_trace()

        vocab_tmp['<start>'] = len(vocab_tmp)
        vocab_tmp['<end>'] = len(vocab_tmp)
        vocab_tmp['<eof>'] = len(vocab_tmp)

        # 4 instead of 3 because the last one is emb of unk
        for i in range(4):
        ...
        # import pdb
        # pdb.set_trace()
        assert len(vocab_tmp) + 1 == embeddings_tmp.size()[0], f"vocab_dic and embedding size not match!"

        self.vocab = vocab_tmp
        self.embedding = torch.nn.Embedding(embeddings_tmp.shape[0], embeddings_tmp.shape[1])
        self.embedding.weight = torch.nn.Parameter(embeddings_tmp)
        if not fine_tune:
            self.embedding.weight.requires_grad = False
        else:
            self.embedding.weight.requires_grad = True

        print(f'total {len(self.vocab)} words.')
        ##################### ! Added here ########################
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
        ...
        for word_len in range(2, self.max_len + 1):
            word_indices: List[int] = []
            pre_tokens = [
                ''.join([token.text for token in sentence[max(0, word_idx - word_len + 1): word_idx + 1]]) \
            ...
            mask = torch.tensor(
                [
                    (1. if (len(token) == word_len and word_indices[idx] != len(self.vocab)) else 0.) \
                    for idx, token in enumerate(pre_tokens)
                ]
            )
            try:
                ...
                embeddings += self.embedding(torch.tensor(word_indices, dtype=torch.long, device=self.device)) * mask[:, None]
            except:
                pdb.set_trace()

            word_indices: List[int] = []
            after_tokens = [
                ''.join([token.text for token in sentence[word_idx: word_idx + word_len]]) \
                for sentence in sentences \
                for word_idx in range(len(sentence))
            ]
            ...
            mask = torch.tensor(
                [
                    (1. if (len(token) == word_len and word_indices[idx] != len(self.vocab)) else 0.) \
                    for idx, token in enumerate(after_tokens)
                ]
            )
            if not self.force_cpu:
                mask = mask.to(self.device)
    ...
    def train(self, mode=True):
        if not self.fine_tune:
            ...
            super(FastChineseWordEmbeddings, self).train(mode)

    def to(self, device):
        if self.force_cpu:
            device = torch.device("cpu")
        self.device = device
        super(FastChineseWordEmbeddings, self).to(device)
    ...
    def _apply(self, fn):
        if fn.__name__ == "convert" and self.force_cpu:
            ...
            if not hasattr(self, "device"):
                self.to(flair.device)
            return
        super(FastChineseWordEmbeddings, self)._apply(fn)
...
class RandomEmbeddings(TokenEmbeddings):
    """Standard static word embeddings, such as GloVe or FastText."""

    def __init__(self, all_vocab: List[str], embedding_length: int, field: str = None, fine_tune: bool = True, force_cpu: bool = False,
                 stable: bool = False):
        """
        Initializes classic word embeddings. Constructor downloads required files if not there.
        :param embeddings: one of: 'glove', 'extvec', 'crawl' or two-letter language code or custom
        ...
        set stable=True to use the stable embeddings as described in https://arxiv.org/abs/2110.02861
        """
        embeddings = f'Randomly initialized {embedding_length}d embs.'
        self.embeddings = embeddings
        ...
        self.embedding = torch.nn.Embedding(embedding.shape[0], embedding.shape[1])
    ...
    def train(self, mode=True):
        if not self.fine_tune:
            ...
            super(RandomEmbeddings, self).train(mode)

    def to(self, device):
        if self.force_cpu:
            device = torch.device("cpu")
        self.device = device
        super(RandomEmbeddings, self).to(device)
    ...
    def _apply(self, fn):
        if fn.__name__ == "convert" and self.force_cpu:
            ...
            if not hasattr(self, "device"):
                self.to(flair.device)
            return
        super(RandomEmbeddings, self)._apply(fn)
...
class FastWordEmbeddings(TokenEmbeddings):
    ...
    def __init__(self, embeddings: str, all_vocab: List[str], field: str = None, fine_tune: bool = False, force_cpu: bool = True,
                 stable: bool = False):
        """
        Initializes classic word embeddings. Constructor downloads required files if not there.
        :param embeddings: one of: 'glove', 'extvec', 'crawl' or two-letter language code or custom
    ...
    def train(self, mode=True):
        if not self.fine_tune:
            ...
            super(FastWordEmbeddings, self).train(mode)

    def to(self, device):
        if self.force_cpu:
            device = torch.device("cpu")
        self.device = device
        super(FastWordEmbeddings, self).to(device)
    ...
    def _apply(self, fn):
        if fn.__name__ == "convert" and self.force_cpu:
            ...
            if not hasattr(self, "device"):
                self.to(flair.device)
            return
        super(FastWordEmbeddings, self)._apply(fn)
...
class TransformerWordEmbeddings(TokenEmbeddings):
    ...
    def __init__(
        ...
        self.context_inverted_index = context_inverted_index
        """ !Added """
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
        ...
        if self.context_length > 0:
            ...
            if self.context_data and hasattr(sentences[0], 'similar_sentence_ids'):
                for sent in sentences:
                    # if sent.is_context_set(): continue
                    context_count = 2
                    # ids = random.choices(sent.similar_sentence_ids, k=context_count)
                    ids = sent.similar_sentence_ids[:context_count]

                    previous_ids = ids[:context_count // 2]
                    next_ids = ids[context_count // 2:]

                    prev_sent = sent
                    for idx in previous_ids:
                        sent_tmp = Sentence(self.context_data[idx])
                        prev_sent._previous_sentence = sent_tmp
                        prev_sent = sent_tmp

                    next_sent = sent
                    for idx in next_ids:
                        sent_tmp = Sentence(self.context_data[idx])
                        next_sent._next_sentence = sent_tmp
                        next_sent = sent_tmp
                    # pdb.set_trace()   
            else:
